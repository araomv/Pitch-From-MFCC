function [f0_time,f0_value,SHR,f0_candidates]=shrp_spec2(Y,Fs,F0MinMax,frame_length,timestep,SHR_Threshold,ceiling,med_smooth,CHECK_VOICING)
...
cur_cand1=0;
cur_cand2=0;
% textprogressbar('SHRP SPEC');
for n=1:nf
%     textprogressbar(n*100/nf);
    segment=Y(:,n);
    curtime=f0_time(n);
    if voicing(n)==0
        curf0=0;
...
%*****************************************************************************************
function [interp_amplitude]=GetLogSpectrum(amplitude,fftlen,limit,logf,interp_logf)
